...
QVariant ProcessingParameterTable::data(const QModelIndex &index, int role) const
{
    ...
    int x=m_data.value(index.row(), ProcessingTask::ProcessingItem()).location;
    if (role==Qt::DisplayRole || role==Qt::EditRole) {
        if (index.column()==0) {
            if (mode==ProcessingTask::Mode::XZ) return "XZ";
            if (mode==ProcessingTask::Mode::ZY) return "ZY";
        } else if (index.column()==1) {
            if (mode==ProcessingTask::Mode::ZY) return std::max(0, x);
        } else if (index.column()==2) {
            if (mode==ProcessingTask::Mode::XZ) return std::max(0, x);
        }
    }
    return QVariant();
}
        ...
        if (role==Qt::DisplayRole || role==Qt::EditRole) {
            if (index.column()==0) {
                if (value.toString().toLower()=="xz" || value.toString().toLower()=="zx") {
                    m_data[index.row()].mode=ProcessingTask::Mode::XZ;
                    ok=true;
                }
                if (value.toString().toLower()=="yz" || value.toString().toLower()=="zy") {
                    m_data[index.row()].mode=ProcessingTask::Mode::ZY;
                    ok=true;
                }
            } else if (index.column()==1) {
                if (mode==ProcessingTask::Mode::ZY) {
                    m_data[index.row()].location=value.toInt();
                    ok=true;
                }
            } else if (index.column()==2) {
                if (mode==ProcessingTask::Mode::XZ) {
                    m_data[index.row()].location=value.toInt();
                    ok=true;
                }
            }
